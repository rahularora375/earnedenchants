...
public class BuiltInRegistries {
	...
	private static final Map<Identifier, Supplier<?>> LOADERS = Maps.<Identifier, Supplier<?>>newLinkedHashMap();
	...
	private static <T, R extends WritableRegistry<T>> R internalRegister(
		...
		Bootstrap.checkBootstrapCalled(() -> "registry " + resourceKey.identifier());
		Identifier identifier = resourceKey.identifier();
		LOADERS.put(identifier, (Supplier)() -> registryBootstrap.run(writableRegistry));
	...
	private static <T extends Registry<?>> void validate(Registry<T> registry) {
		registry.forEach(registry2 -> {
			...
			if (registry2 instanceof DefaultedRegistry) {
				...
				Objects.requireNonNull(registry2.getValue(identifier), "Missing default of DefaultedMappedRegistry: " + identifier);
			}
		});
	}
